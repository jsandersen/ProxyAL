...
class ActiveLearner(ABC):
    ...
    def __init__(self, warmstart_size, X_pool, y_pool, X_test, y_test, X_pool_index):
    ...
    def run_warmstart(self):
        ...
        closest, _ = vq(kmeans.cluster_centers_, self.X_pool)
...
def unc_least_confident(proba_pool):
    return 1 - np.array(proba_pool).max(axis=1)
...
class SKLearnActiveLearner(ActiveLearner):
    ...
    def __init__(self, warmstart_size, X_pool, y_pool, X_test, y_test, X_pool_index, Model, model_args = {}):
        self.Model = Model
    ...
    def _query(self, X_pool, step_size):
        ...
        unc = unc_least_confident(proba_pool)
